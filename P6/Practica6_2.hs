...
profundidad :: Arbol a -> Int
profundidad (H x) = 0
profundidad (N x i d) = 1 + maximum [(profundidad i), (profundidad d)]
...
anadeHojas :: Arbol a -> a -> a -> Arbol a
anadeHojas (H x) a b = (N x (H a) (H b))
anadeHojas (N x i d) a b = (N x (anadeHojas i a b) (anadeHojas d a b))
...
ramifica :: ArbolG a -> ArbolG a -> (a -> Bool) -> ArbolG a
ramifica (NG x abs) a2 p
      | p x = NG x (a2:[ramifica a a2 p | a <- abs])
      | otherwise = NG x [ramifica a a2 p | a <- abs]
...
nHojasG :: ArbolG a -> Int
nHojasG (NG _ []) = 1
nHojasG (NG _ abs) = sum [nHojasG a | a <- abs]
-- nHojasG (NG _ abs) = sum $ map nHojasG abs
...
profundidadG (NG _ []) = 0
profundidadG (NG _ abs) = 1 + (maximum (map profundidadG abs))
...
bin2gen (H x) = NG x []
bin2gen (N x i d) = NG x [bin2gen i, bin2gen d]
