...
buscaClave s (a:as)
    | (clave a) == s    = a
    | otherwise         = buscaClave s as

-- ---------------------------------------------------------------------
-- Ejercicio 3.2 Definir la función (siguienteNodo s as), que reciba una
...
siguienteNodo s as = (buscaClave s as, noCoinciden)
    where noCoinciden = [a | a <- as, (clave a) /= s]
...
inserta (NT c hs) ("", v) = NT c ((HT v):hs)
inserta (NT c hs) (cs, v) = NT c ((inserta coincide (tail cs, v)):noCoinciden)
    where
        (coincide, noCoinciden) = siguienteNodo [head cs] hs
...
--insertaElemsEnArbol a (c:cs) = insertaElemsEnArbol (inserta a c) cs
...
consultaValor (NT _ hs) "" = [v | (HT v) <- hojas]
    where hojas = filter esHoja hs
consultaValor (NT c hs) ss
    | null coincidencias = []
    | otherwise          = consultaValor (head coincidencias) (tail ss)
    where coincidencias = [h | h <- hs, (clave h) == [head ss]]
