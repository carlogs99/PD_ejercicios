...
mezclaColas :: Cola a -> Cola a -> Cola a
mezclaColas c1 c2 = mezclaColasAux c1 c2 vacia True
    where
        mezclaColasAux c1 c2 ac pri
            | (esVacia c1) && (esVacia c2)  = ac
            | esVacia c1                    = mezclaColasAux vacia vacia (ponAlaCola ac c2) True
            | esVacia c2                    = mezclaColasAux vacia vacia (ponAlaCola ac c1) True
            | pri                           = mezclaColasAux rc1 c2 (inserta pc1 ac) False
            | otherwise                     = mezclaColasAux c1 rc2 (inserta pc2 ac) True
            where
                pc1 = primero c1
                pc2 = primero c2
                rc1 = resto c1
...
agrupaColas :: [Cola a] -> Cola a
agrupaColas cs = agrupaColasAux cs vacia
    where
        agrupaColasAux [] ac = ac
        agrupaColasAux (c:cs) ac = agrupaColasAux cs (mezclaColas ac c)
...
perteneceCola y c 
    | esVacia c     = False
    | otherwise     = (pc == y) || perteneceCola y rc
    where
        pc = primero c
        rc = resto c
...
ordenadaCola :: Ord a => Cola a -> Bool
ordenadaCola c
    | (esVacia c) || (esVacia rc) = True
    | (pc < prc)                  = ordenadaCola rc
    | otherwise                   = False
    where
        pc = primero c
        rc = resto c
...
maxCola c
    | esVacia c = error "Cola vacia"
    | otherwise = maxColaAux c (primero c) True
        where 
            maxColaAux c m pri
                | esVacia c = m
                | pri       = maxColaAux rc pc False
                | pc <= m   = maxColaAux rc m False
                | otherwise = maxColaAux rc pc False
                where
                    pc = primero c
                    rc = resto c
