...
-- La definición es
perfectos :: Int -> [Int]
perfectos n = [x | x <- [1..n-1], x == ((sum (factores x)) - (last (factores x)))]
...
-- La comprobación es 
-- *Main> quickCheck prop_conmutativa_productoEscalar 
-- +++ OK, passed 100 tests.
...
euler1 :: Integer -> Integer
euler1 n = sum [x | x <- [3..n-1], (mod x 3) == 0 || (mod x 5) == 0]
...
prop_replica n x = (replica n x) == (replicate n x)
...
numeroAbundante n = n < sum (factores n) - last (factores n)
...
todosPares n = ((length [x | x <- (numerosAbundantesMenores n), even x]) == 
...
primerAbundanteImpar = head [x | x <- [1..], ((numeroAbundante x) && (odd x)) == True]
...
linea n = [primero..primero+(n-1)]
    where primero = suma (n-1) + 1
...
circulo :: Float -> Int
circulo n = sum [1 | x <- [0..n], y <- [0..n], (x^2 + y^2) < (n^2)]
...
ocurrenciasDelMaximo :: (Num a, Ord a) => [a] -> (a, Int)
ocurrenciasDelMaximo [] = (0,0)
ocurrenciasDelMaximo xs = (maximum xs, sum [1 | x <- xs, x == (maximum xs)])
...
tienenS :: [[Char]] -> [Int]
tienenS xss = [length xs | xs <- xss, 's' `elem` xs || 'S' `elem` xs]
...
algoOrdenada xs = and [x1 <= 2*x2 | (x1, x2) <- zip xs (tail xs)]

-- alternativa:
-- algoOrdenada xs = null [1 | (x1, x2) <- zip xs (tail xs), x1 > 2*x2]
...
tripletas :: [a] -> [[a]]
tripletas xs = [[x,y,z] | ((x,y),z) <- zip (zip xs (tail xs)) (tail (tail xs))]

-- alternativa con funcion zip3:
-- tripletas xs = [[x,y,z] | (x,y,z) <- zip3 xs (tail xs) (tail . tail xs)]

-- otra alternativa usando indices:
-- tripletas xs [[xs !! i, xs !! (i+1), xs !! (i+2)] | i <- [0..length xs -3]]]
...
unitarios :: Int -> [Int]
unitarios x = [numeroDesarrollado n x | n <- [1..]]

numeroDesarrollado :: Int -> Int -> Int
numeroDesarrollado n x = sum [x*10^e | e <- [0..(n-1)]] 
...
minLargoLista :: [[Char]] -> Int
minLargoLista xs = minimum [length x | x <- xs]

cortas :: [[Char]] -> [[Char]]
cortas xs = [x | x <- xs, length x == minLargoLista xs]
...
esLibreDeCuadrado a = and [(mod a (i^2) /= 0) | i <- [2..(div a 2)]]

libresDeCuadrado :: Int -> [Int]
libresDeCuadrado n = take n ([x | x <- [1..], esLibreDeCuadrado x])
