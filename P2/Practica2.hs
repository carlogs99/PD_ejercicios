...
-- La comprobación es
{- *Main> quickCheck prop_xor_equivalentes 
+++ OK, passed 100 tests. -}
...
-- El caracter $ es equivalente a un parentesis desde el $ hasta el final
...
-- init toma todos los elementos menos el ultimo
...
numeroMayor :: (Num a, Ord a) => a -> a -> a
numeroMayor x y = max (x*10 + y) (x + y*10)
...
raices a b c
    ...
    | (numeroDeRaices a b c) == 2   = [(-b+dis)/(2*a), (-b-dis)/(2*a)]
    where dis = b^2 - 4*a*c
